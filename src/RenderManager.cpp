...
void RenderManager::setCamera(std::string item)
{
	SceneNode* head_node = scene_manager->getSceneNode("Head Transform Node");
	if(item.compare("Inside Camera") == 0)
	{
		head_node->setPosition(2.15,.1,-.6);
	}
	else
	{
		head_node->setPosition(-3,.5,-.6);
	}
}

void RenderManager::changeStation(std::string item)
{
	game_manager->playRadio(item);
}
...
void RenderManager::keyPressed(std::string game_key)
{
	...
	SceneNode* scene_ship_node = (SceneNode*) root->getChild("Spaceship Transform Node");
	SceneNode* animation_node = (SceneNode*) scene_ship_node->getChild("Spaceship Animation");
   SceneNode* scene_rotate_node = (SceneNode*) animation_node->getChild("Rotate Transform");
	const Quaternion& current_quat = scene_rotate_node->getOrientation();
	Vector3 x_axis(1,0,0);
	if(game_key == "A")
	{
		Quaternion left_quat(Degree(-10), x_axis);
		scene_rotate_node->rotate(left_quat);
	}
	else if(game_key == "D")
	{
		Quaternion left_quat(Degree(10), x_axis);
...
void RenderManager::mouseMoved(uint32 mouse_x, uint32 mouse_y, int mouse_rel_x, int mouse_rel_y)
{
      //get root's (one) child and change its rotation based on mouse movement
	   ...
	   SceneNode* animation_node = (SceneNode*) scene_ship_node->getChild("Spaceship Animation");
	   SceneNode* scene_rotate_node = (SceneNode*) animation_node->getChild("Rotate Transform");
	   SceneNode* scene_head_node = (SceneNode*) scene_rotate_node->getChild("Head Transform Node");
	   const Quaternion& current_quat = scene_head_node->getOrientation();

	   Vector3 x_axis(1, 0, 0);
	   Vector3 y_axis(0, 1, 0);
	   Vector3 z_axis(0, 0, 1);
	   int x_degree = 0 - mouse_rel_x;
	   int y_degree = 0 - mouse_rel_y;

	   Quaternion x_quat(Degree(y_degree), x_axis);
	   Quaternion y_quat(Degree(x_degree), y_axis);
	   Quaternion z_quat(Degree(y_degree), z_axis);
	   Quaternion new_quat = y_quat*current_quat;  //quaternions to the right are applied first
	   //scene_parent_node->rotate(new_quat);  //here, new_quat is applied to the right
	   scene_head_node->setOrientation(new_quat);
...
void RenderManager::processAnimations(float time_step)
{
   ...
   while(anim_iter->hasNext())
   {
	  ...
	  if(animation_state->getTimePosition() > 7.1 && animation_state->getTimePosition() < 8)
		  game_manager->playAudio(19, 0);
   }
   delete anim_iter;
}
...
void RenderManager::addSceneNodeChildren(TiXmlNode* xml_node, SceneNode* parent_node, float* values)
{
   ...
   //be able to parse a camera as a child
   TiXmlNode* curr_child_camera_xml = xml_node->FirstChild("camera");
	if(curr_child_camera_xml)
	{
		std::string camera_name_text = GameManager::textFromChildNode(curr_child_camera_xml, "name");
		SceneNode* camera_node = scene_manager->createSceneNode(camera_name_text);
		parent_node->addChild(camera_node);
		
		std::string camera_position_text = GameManager::textFromChildNode(curr_child_camera_xml, "position");
		GameManager::parseFloats(camera_position_text, values);
		Vector3 camera_position(values[0], values[1], values[2]);
		camera->setPosition(camera_position);

		std::string camera_look_at_text = GameManager::textFromChildNode(curr_child_camera_xml, "look_at");
		GameManager::parseFloats(camera_look_at_text, values);
		Vector3 camera_look(values[0], values[1], values[2]);
		camera->lookAt(camera_look);

		std::string camera_clip_distance_text = GameManager::textFromChildNode(curr_child_camera_xml, "clip_distance");
		GameManager::parseFloats(camera_clip_distance_text, values);
		camera->setNearClipDistance(values[0]);
		camera->setFarClipDistance(values[1]);
