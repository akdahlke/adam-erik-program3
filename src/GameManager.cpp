...
void GameManager::playRadio(std::string channel) 
{
	srand(time(NULL));
	int random;
	audio_manager->stopStream();
	
	if (channel.compare("FalloutStation") == 0) {
		//random between 20-22 (Fallout Radio)
		random = rand() % 3 + 20;
		playAudio(random, 0);
	}
	else if (channel.compare("EpicStation") == 0) {
		//random between 23-25 (VG soundtracks)
		random = rand() % 3 + 23;
		playAudio(random, 0);
	}
	else if (channel.compare("StyxStation") == 0) {
		//random between 26-29 (Styx)
		random = rand() % 4 + 26;
		playAudio(random, 0);
	}
	else if (channel.compare("TrollStation") == 0) {
		playAudio(30, 0);
	}
}
